...
def get_coordinates_with_intensity():
    ...
    total_GHI2 = 0
    total_GHI3 = 0
    persentase_luas1 = 0  # Inisialisasi persentase luas
    ...
    if selectedMonth == 'February' and selectedYear1 in [2012, 2016, 2020]:
        days_in_month['February'] = 29  # Fixed the typo
        ...
        if geometry['type'] == 'Polygon':
            ...
            for lat, lon in grid_coords:
                ...
                if intensity is not None:
                    total_GHI3 += intensity  # Menambahkan GHI ke total
                    total_GHI2 = total_GHI3 * 8
                    logging.info(f"Total GHI2 after ({lat}, {lon}): {total_GHI2}")
                    
    if selectedMonth in days_in_month:
        total_GHI1 = total_GHI2 * days_in_month[selectedMonth] / 1000
    logging.info(f"Total GHI1 after ({lat}, {lon}): {total_GHI1}")
    persentase_luas1 = global_persentase_luas
    logging.info(f"persentase luas: {persentase_luas1}")
    total_GHI = total_GHI1 * persentase_luas1
...
@app.route('/analyze_polygon_image_area_terbuka', methods=['POST'])
def analyze_polygon_image_area_terbuka():
    # Baca gambar
    ...
    # Hitung persentase luas area terbuka yang tersedia
    if luas_keseluruhan > 0:
        persentase_luas = (luas_tertentu / luas_keseluruhan) * 100
    else:
        persentase_luas = 0
        
    # Simpan persentase luas ke dalam variabel global
    global global_persentase_luas
    global_persentase_luas = persentase_luas
...
@app.route('/analyze_polygon_image_atap', methods=['POST'])
def analyze_polygon_image_atap():
    # Baca gambar
    gambar = cv2.imread('cropped_map_image.png')
    ...
    # Rentang warna atap dalam ruang warna RGB
    lower_color = np.array([240, 232, 216])  # Nilai minimum RGB untuk atap
    upper_color = np.array([248, 241, 230])  # Nilai maksimum RGB untuk atap

    # Hitung luas area dengan warna atap
    mask, contours_tertentu, luas_tertentu = hitung_luas_area_tertentu(gambar, lower_color, upper_color)
